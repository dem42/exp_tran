...
{ \huge \bfseries Tensor-based Deformable Models for the Recognition, Synthesis and Transfer of Facial Expressions}\\[1cm]
\LARGE by\\[1cm]
...
Deformable models allow computers to understand dynamic objects. To enable a
machine to perform a complex task on real world objects, it must
have an understanding of the attributes of these objects. Using a deformable model, this
understanding is provided as a set of parameter values. Given a deformable model
of a human face, the computer can be programmed to recognize or even synthesize
novel face shapes by adjusting these parameters. Tensor-based models enhance this ability by making it
possible to separately manipulate the attributes of the deformable object. This
important quality of separability makes it possible to easily transfer deformations
between objects. We develop an application that understands a face shape using
its tensor-based deformable model. In this face shape model, the sources of deformations are given by
an identity and an expression attribute. The tensor model encodes each of the
attributes using a separate parameter vector. We can therefore transfer the
deformations caused by a change in expression by transferring the corresponding
parameters. The tensor model makes it possible to conveniently generate novel
faces by simply choosing a set of parameter values, which can be useful in the field of 3D
animation. However, setting the parameters manually to achieve complex deformations can
be difficult. We therefore derive a fitting algorithm to automatically locate
the model parameter values from an image
of an object.
...
First of all, I would like to thank my supervisor, Professor Duncan Gillies
for his help, encouragement and invaluable feedback throughout the course of the
project. I would also like to thank my friends for providing me with much needed
distractions from work. Finally, I would like to thank
my parents for their continuous support and for helping shape me into who I am
today.
...
\textit{expression transfer}. The primary purpose of expression transfer is to change the expression of a subject independent of other attributes
that define the shape or texture of his or her face. The goal is to be
able to capture
...
This paper describes the development of an expression transfer application, which combines motion tracking and a deformable
model to gain understanding of the face deformations present in the input video
recording. Using this understanding, the deformations can then be transferred to
another instance of the model. The application allows the user to separately manipulate the identity and
expression of a face shape. Furthermore, the user can locate the parameters of
the face shape model using the model fitting algorithms provided by the application.
...
\item Elucidated a multilinear deformable model designed for expression
...
robust enough to be able to deal with variations caused by noise.
...
on the data tensor. The Tensorfaces are given by $\mathcal{S}\times_1
\mathbf{U}_1$. They are essentially the the eigenvectors of
$\mathbf{U}_1$ transformed into eigenmodes using the core tensor. Vlasic, Brand, Pfister and Popovic \cite{faceTransfer05} build a
...
If the image plane were to be placed in front of the camera then the ratio given
...
as a multiplication of a 3D point in Cartesian coordinates with a matrix. To obtain a matrix formulation, we must
...
camera equipment the image plane will usually have dimensions specified in millimeters. The values
...
projection, parallel lines remain parallel after being projected into the image plane.
...
we make the assumption that all these depths are equal. Then a perspective projection
...
are orthogonally projected onto this new plane. Finally, we perspectively project
...
The weak perspective transformation is a linear
...
rotation around the $(X,Z)$ plane needs to be described by $R_y(\psi)$. The
pre-multiplication of a point $\mathbf{x} = (x,y,z)^T$  with the
...
is decomposed into its $x$ and $y$ components which are then projected onto the rotated
...
appropriate angles. Since the total rotation matrix is a product of three
...
point $M_0$ can be conveniently picked as one of the known feature
...
The POSIT algorithm first makes a rough estimate of these values using a \textit{pose from
  orthography and scaling} (POS) step. In the POS step it is assumed that the
points are far enough from the camera so that we can use a weak-perspective
camera model. With this camera model the values $i$, $j$ and $Z_0$ all have
closed form solutions. After this first step, the 2D image points are then projected back into the object
coordinate space, this time using the true perspective camera and the pose (rotation,
translation) estimated in the POS step. These new points are then used in
further  POS steps until the algorithm converges, hence the name POS with iterations
...
either keep locating the points in every frame or to track the movement of the points
themselves. This problem of tracking the movement of points or objects between
...
The problem of registering a displacement between two images is
...
and we are trying to locate the displacement of the apple.
...
given by
\begin{equation}
E(\mathbf{h}) = \int_{\mathcal{W}} (F(x + \mathbf{h}) - G(x))^2 \mathrm{d}x
\end{equation}

The naive way to locate this $\mathbf{h}$  would be to iterate through all the possible values of $\mathbf{h}$ and measure the $E(\mathbf{h})$ norm. A
...
2\int_{\mathcal{W}} \mathbf{g}(F(x) + \mathbf{g}^T\mathbf{h} - G(x))
\mathrm{d}x
\end{align}
Setting the above to zero and rearranging the terms we get the fundamental
equation which relates the intensity change to the optimal displacement
$\mathbf{h}$ as
\begin{align}\label{eq:kanade3}
\Bigr(\int_{\mathcal{W}} \mathbf{g}\mathbf{g}^T\mathrm{d}x\Bigl)\mathbf{h} = \int_{\mathcal{W}} \mathbf{g}(F(x) - G(x))
\mathrm{d}x
\end{align}
...
computed and the guess is propagated to the lower of the pyramid.
...
function. Assume we are given a linear system of equations defined in matrix notation by
...
\mathbf{V\Sigma}^{-1}\mathbf{U}^T$. If the square matrix $\mathbf{A}$ is not invertible it
...
The ultimate goal of the application is to transfer the expressions between two
...
implementations of algorithms. The disadvantage is that some frameworks may
...
compatibility. To design a portable application only cross-platform
...
The explanation we found for why only non-negative parameters produce
unrealistic faces is based on the fact that all points on a face scan expand
when the face is moved in the
positive direction. Some points may expand faster, others slower, but this ratio
stays constant meaning that points will not escape no matter how much the face
is scaled. A positive linear combination of the original face
scans thus leads to larger faces which are still realistic. On the other hand,
when the faces are subtracted from each other during linear combination, then
some points may expand and others may shrink which causes unrealistic face shapes.
...
To transfer an expression given two instances of the tensor-based shape model,
we take the expression parameters of the source instance and the identity
parameters of the target instance and generate a face with the transferred
expression. However, it is important to note that the model accounts for the
fact that different individuals may perform a certain type of expression
differently. This means that the transferred frown may look different in the
newly generated face. For the purpose of this application, the new expression
is however very good approximation.
...
In summary, the tensor based shape model allows the user to easily synthesize
realistic expressions. Thereby, it is made possible for casual users and
beginner animators to generate believable 3D models of faces. These models then
...
is impossible to determine the correct expression and 3D pose using the
...
point change, the ones far from the point may not and the evidence from the many
far
off textures will outweigh the small contribution from the few nearby
...
The performance of the fitting algorithm is determined primarily by the following four factors
\begin{itemize}
...
and improved optimization result on an interesting frame of a 100 frame video
sequence optimized using a constrained non-negativity minimization scheme.
...
necessary. The identity depends on a large quantity of small details such as the width of the nose, the
fullness of lips and others. A user cannot manually select all these features.
...
frames or points are used, making the penalty value caused by the regularization
parameter smaller by comparison. Constrained optimization is easier to use. However, a fitting algorithm utilizing constrained optimization has weaker
...
\caption{The convergence behavior of the two fitting algorithm
...
a small $\lambda = 20$. On the other hand, the best result was obtained with
constrained optimization. 

\begin{figure}[H]
\centering
\subfloat[Regularization $\lambda=20$.]{\includegraphics[scale=0.23]{images/angry_reg20.png}}$\;$
\subfloat[Regularization $\lambda=60$.]{\includegraphics[scale=0.23]{images/angry_reg60.png}}\\
\subfloat[Regularization $\lambda=100$.]{\includegraphics[scale=0.23]{images/angry_reg100.png}}$\;$
...
keep the mouth on the model open when fitting the model to an image. This is due to
the fact that the mean face from the Binghamton database has an open mouth. Since the regularization algorithm
penalizes any large discrepancy from the mean face, it therefore often falsely
...
\subfloat[The regularized fitting algorithm falsely assumes the mouth is open, since this minimizes error better than an open mouth.]{\label{fg:meanface}\includegraphics[scale=0.35]{images/reg_mouth_open.png}}\\
...
has to be done by trial and error. Secondly, the fitting algorithm often falsely
...
In summary, we have discussed and explored two approaches to avoid
...
shapes for only certain values of the regularization parameter $\lambda$. However, this
parameter must often be found by trial and error. The parameter's magnitude
...
faces. The constrained fitting algorithm does not require any input parameters
...
means that the multi-frame coordinate descent algorithm has to find an expression
...
algorithm. This constraint limits the algorithm's performance, since it requires that we solve a large linear system, obtained by
collecting the feature points from all frames. 

\section{Expression Transfer Results}
Given a 3D model of the source subject and a 3D model of the target subject, the
final task is to transfer the source expression to the target and then reanimate
...
correct appearance. We therefore must utilize shape information to transfer
texture. 

This can be done by finding the texture coordinates through projection of the newly generated face into target the image and using the image as
...
noticeably skewed texture in the target image.
...
clone the features from the source texture into the target texture. In order for
the
result to seem realistic, this is done with seamless blending to produce results
...
Since the geometry of both 3D models is known from the parameters obtained using
the fitting algorithm, the
transfer is done by simply taking the smile texture from the source image and cloning it
over the mouth on the target image. This approach was also utilized by Vlasic et
...
\subfloat[Step 2: Transfer of the beginnings of a smile.]{\includegraphics[scale=0.35]{images/poisson4.png}}
\caption{Texture transfer using Poisson cloning. Depicted is a case where the
  technique does not work well due to the discrepancy in size.}\label{poisson3}
\end{figure}
...
The performance of both versions of the fitting algorithm is therefore good enough
to make the application accessible for casual users. The slower, per frame runtime of the non-negativity least squares algorithm is due to the
fact that it performs a sequence of iterations until convergence during every parameter update step in the
coordinate-descent expression transfer algorithm. The linear
...
of iterations is not constrained it may sometimes take the regularized algorithm longer to
converge to a stable solution.


\chapter{Conclusion}
We developed an expression transfer application which allows a
casual user to construct and explore a tensor based 3D model of faces shapes. The user
can use the application to either synthesize novel faces or to manipulate and
change the expression and identity of an existing face using a simple OpenGL
based GUI. The most important feature of the application is to transfer the
expressions from one video sequence to another. The functionality of the
application enables the user to easily generate and manipulate 3D face shapes
and video sequences, making it useful for 3D animators and for casual users alike.

The tensor based shape model is constructed using the Binghamton database of 3D face
scans. This is done by first aligning the face scans into a data tensor and
...
shape the generative function may include 3D pose parameters in form of a rotation
...
independently. Using sliders in the GUI a user can create new 3D faces and
change their identity or expression. When a more complex expression is required
it may become difficult to correctly adjust all the sliders to generate this
expression.

To find complex expression and identity parameters, application utilizes a fitting algorithm to support
...
in a target video recording. Since the tensor model only encapsulates shape
...
pattern. Since performance is key to a user, the expression transfer application
makes use of multi-threading so that multiple fitting algorithm invocations may
...
with more attributes than just the identity and expression. For example it would be
interesting to build an appearance tensor model with attributes for expressions,
...
be possible to contribute to the deformable modeling field by exploring tensor models
...
need for expensive equipment. Structured light based scanning equipment and other 3D shape recovery approaches
...
more variability could be introduced to an existing databases by utilizing a FEM based
model to generate new faces from the ones in the database and using these to expand the database.
...
and they are already being utilized as feature points in the application. Even an external feature point tracker could therefore be
...
could be overcome by utilizing an appearance model. Since this
model generates a textured representation of the face its projection into
2D is a textured image of the face. It is, therefore, less constrained in how the
error is computed. An intriguing future project could thus involve deriving a
fitting algorithm which would not require point to point correspondences. To
construct a tensor-based appearance model it would be necessary to known the
RGB values at every vertex, which means that a database with texture maps and texture
coordinates would be required. A tensor-based model of appearance would also
...
approaches and derives formulas for two of the four optimal parameters.
