...
accomplished by correctly modeling the shape of an individual's face.
...
The model point $\mathbf{p}_i$ components are three successive components of the vector
...
which is being projected using the camera matrix. In equation \ref{eq:fit8}, $\gamma$ is therefore a function of the model parameters and the rotation
...
space. This projection can be inverted to give the reprojection from image space into
...
class. Persistence means the tensor model needs to be computed only once. After
...
\begin{figure}[H]
\centering
\subfloat[Moving a happy face in the negative neutral direction from right to left.]{\label{sfg:happyneg}\includegraphics[scale=0.25]{images/sub_neu_from_happy.png}}\\
\subfloat[Moving a neutral face in the negative happy direction from right to left.]{\label{sfg:neghappy}\includegraphics[scale=0.25]{images/sub_happy_from_neutral.png}}
\label{fg:faceaddsub}
...
\begin{figure}[H]
\centering
\subfloat[Interpolating between two identities with a neutral expression.]{\includegraphics[scale=0.23]{images/ident_interpol.png}}\\
\subfloat[Interpolating between two identities with a happy expression.]{\includegraphics[scale=0.23]{images/id_inerpol_happy.png}}
\label{fg:faceidinter}
\caption{Linear interpolation between subjects from the Binghamton database.}
\end{figure}
...
\subfloat[Movement of feature points when interpolating between a neutral and
  fearful expression.]{\includegraphics[scale=0.23]{images/fear.png}}\\
\subfloat[Movement of feature points when interpolating between a surprised and
  happy expression.]{\includegraphics[scale=0.23]{images/happy_surprise.png}}
\label{fg:featureinter}
\caption{The movement of feature points (designated by red spheres).}
\end{figure}
