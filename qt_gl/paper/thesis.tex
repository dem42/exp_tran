...
analysis to build a model it is necessary to have a dataset to analyze. The less
sophisticated models attempt to model the variation in the shape of an
object. The more sophisticated ones perform statistical analysis on the entire
appearance of the object -- including the texture, lighting etc. 

A \textit{shape model} describes the boundaries of an object. For instance, a
...
\subsubsection{Statistical Models of Appearance}
...
The active shape model (ASM) proposed by Cootes and Taylor \cite{activeApp04} is based
...
The translation vector is specified as the vector $\vec{OM_0}$ between the
...
as $t = \frac{Z_0}{f}\vec{Om_0}$ where $m_0$ is the known 2D projection of $M_0$
...
\vec{M_0M_i}\cdot I  = x_i(1+\varepsilon_i) - x_0\\
\label{eq:posit2}
\vec{M_0M_i}\cdot J  = y_i(1+\varepsilon_i) - y_0
\end{gather}
with
\begin{equation*}
I = \frac{f}{Z_0}i \mathrm{, }\quad J = \frac{f}{Z_0}j \quad \mathrm{and} \quad
\varepsilon_i = \frac{1}{Z_0}\vec{M_0M_i} \cdot k
\end{equation*}
...
\frac{1}{Z_0}\vec{M_0M_i} \cdot k$ with the $k$ and $Z_0$ as guesses from the
...
\subsection{Computing the HOSVD}
The generative tensor model is given by 
...
involved in the computation. In an application with 56 identities, 7 expressions
...
bytes. Clearly the $\mathbf{V}_{exp}$ matrix is too large and exceeds the memory constraints of
