...
Before the complete fitting algorithm can be established two things need to be
addressed. First of all the coordinate descent algoritm discussed sofar was
...
sequence of frames. The identity parameters should
...
For the algorithm to estimate a reasonable approximation to the face in the
video, it is necessary to use a large number of feature points. The user cannot
be expected to enter that many points. It is therefore necessary to generate
a large number of new points to use in addition to the feature points specified
...
For the algorithm to be able to automatically establish the correspondence to a
model point it needs to know the camera matrix, the 3D pose as well as the model
parameters. The feature points also pr
The fitting algorithm for the tensor-based shape model is summarized in the
following steps. 
