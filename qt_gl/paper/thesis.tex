...
the element. This formulation, along with numerical integration techinques for
...
of objects with a known stiffness function to a canditate object in an
...
The idea behind the morhable model is that any linear combination of the
$\mathbf{S_i}$ and $\mathbf{T_i}$ constitues a realistic face. The full
...
To be able to exchange the face we need the $\alpha_i$, $\beta_i$ parameters
...
\item intensity of directed light (3 colours)
\end{itemize}
\begin{itemize}
\setlength{\itemsep}{0pt}
\item intensity of ambient light (3 colours)
\item color contrast
\item gain in each color channel (3 channels)
\item offset in each color channel
\end{itemize}
\end{multicols}
We apply the reconstruction algorithm to estimate the paramters for both images
and then use these parameters to compute
the colour image $I_{model}(x,y)$ of the face we are reconstructing.

The parameters are all
estimated in an analysis-by-synthesis stage in which we attempt to minimise the
difference between the synthetic image $I_{model}$ and the original colour
image $I_{input}$. We measure the quality of this minimisation based on a sum of squared error\\
\begin{equation}
...
This cost function is minimised using a Stochastic Newton Algorithm to
