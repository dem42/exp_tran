...
SVD::SVD(string filename,int f,int e,int v) : filename(filename), n_f(f), n_e(e), n_v(v)
{
        bool loaded = load();
        if(!loaded)
            compute_core_tensor();
...
{	
	...
	matrix_mult(core,m1i,m1e,3*n_v,n_f*n_e,1);
        ...
        for(i=0,j=0;i<3*n_v;i=i+3,j++)
                {
                        face[j].x = (float)(m1e[i][0]);
                        face[j].y = (float)(m1e[i+1][0]);
                        face[j].z = (float)(m1e[i+2][0]);
		}

        //delete[] m1e;
        std::cerr << "got this far" << std::endl;

        kr = new long double*[n_e*n_f];
        for(i=0;i<n_f*n_e;i++)
            kr[i] = new long double[n_e*n_f];
       kron(u2,n_f,n_f,u3,n_e,n_e,kr);
       fstream f("check_inter",fstream::out);
        for(i=0;i<n_v;i++)
            f << face[i].x <<" "<< face[i].y << " " << face[i].z << " ";
...
{    
    ...
    try
    {
        ...
        u2 = new long double*[n_f];
        for(int i=0;i<n_f;i++)
            u2[i] = new long double[n_f];
        u3 = new long double*[n_e];
        for(int i=0;i<n_e;i++)
            u3[i] = new long double[n_e];
        core = new long double*[3*n_v];
        for(int i=0;i<3*n_v;i++)
            core[i] = new long double[n_f*n_e];
